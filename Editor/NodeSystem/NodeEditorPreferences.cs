...
namespace Josephus.NodeSystem.Editor 
{
    ...
    public static class NodeEditorPreferences {
        ...
        public static Settings GetSettings() {
            if (NodeSystem.Editor.NodeEditorWindow.current == null) return new Settings();

            if (lastEditor != NodeSystem.Editor.NodeEditorWindow.current.graphEditor) {
                object[] attribs = NodeSystem.Editor.NodeEditorWindow.current.graphEditor.GetType().GetCustomAttributes(typeof(NodeSystem.Editor.NodeGraphEditor.CustomNodeGraphEditorAttribute), true);
                if (attribs.Length == 1) {
                    NodeSystem.Editor.NodeGraphEditor.CustomNodeGraphEditorAttribute attrib = attribs[0] as NodeSystem.Editor.NodeGraphEditor.CustomNodeGraphEditorAttribute;
                    lastEditor = NodeSystem.Editor.NodeEditorWindow.current.graphEditor;
