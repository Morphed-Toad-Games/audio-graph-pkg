...
namespace Josephus.NodeSystem.Editor 
{
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (NodeSystem.Node) && !scriptType.IsSubclassOf (typeof (NodeSystem.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    NodeSystem.Node node = objs[k] as NodeSystem.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (NodeSystem.NodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                NodeSystem.NodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (NodeSystem.NodeGraph)) as NodeSystem.NodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as NodeSystem.Node)) graph.nodes.Add(objs[u] as NodeSystem.Node);
                }
            }
        }
    }
