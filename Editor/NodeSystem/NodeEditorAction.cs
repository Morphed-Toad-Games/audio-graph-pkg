...
namespace Josephus.NodeSystem.Editor 
{
    public partial class NodeEditorWindow {
        ...
        public static NodeSystem.Node[] copyBuffer = null;
        ...
        private NodeSystem.Node hoveredNode = null;
        [NonSerialized] public NodeSystem.NodePort hoveredPort = null;
        [NonSerialized] private NodeSystem.NodePort draggedOutput = null;
        [NonSerialized] private NodeSystem.NodePort draggedOutputTarget = null;
        [NonSerialized] private NodeSystem.NodePort autoConnectOutput = null;
            ...
            switch (e.type) {
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is NodeSystem.Node) {
                                    NodeSystem.Node node = Selection.objects[i] as NodeSystem.Node;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    NodeSystem.Node node = hoveredPort.node;
                                    NodeSystem.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget)) {
                                NodeSystem.Node node = draggedOutputTarget.node;
                            ...
                            IEnumerable<NodeSystem.Node> nodes = Selection.objects.Where(x => x is NodeSystem.Node).Select(x => x as NodeSystem.Node);
                            foreach (NodeSystem.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.keyCode == KeyCode.A) {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as NodeSystem.Node))) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is NodeSystem.Node).Cast<NodeSystem.Node>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is NodeSystem.Node) {
                    NodeSystem.Node node = item as NodeSystem.Node;
        ...
        public void MoveNodeToTop(NodeSystem.Node node) {
            int index;
        ...
        public void DuplicateSelectedNodes() {
            ...
            NodeSystem.Node[] selectedNodes = Selection.objects.Select(x => x as NodeSystem.Node).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as NodeSystem.Node).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(NodeSystem.Node[] nodes, Vector2 topLeft) {
            ...
            Dictionary<NodeSystem.Node, NodeSystem.Node> substitutes = new Dictionary<NodeSystem.Node, NodeSystem.Node>();
            for (int i = 0; i < nodes.Length; i++) {
                NodeSystem.Node srcNode = nodes[i];
                if (srcNode == null) continue;

                // Check if user is allowed to add more of given node type
                NodeSystem.Node.DisallowMultipleNodesAttribute disallowAttrib;
                ...
                NodeSystem.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++) {
                NodeSystem.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (NodeSystem.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        NodeSystem.NodePort inputPort = port.direction == NodeSystem.NodePort.IO.Input ? port : port.GetConnection(c);
                        NodeSystem.NodePort outputPort = port.direction == NodeSystem.NodePort.IO.Output ? port : port.GetConnection(c);

                        NodeSystem.Node newNodeIn, newNodeOut;
        ...
        public void DrawDraggedConnection() {
            if (IsDraggingPort) {
                ...
                for (int i = 0; i < draggedOutputReroutes.Count; i++) {
                    ...
                    NodeEditorGUILayout.DrawPortHandle(rect, bgcol, frcol, null);
                }
            }
        }

        bool IsHoveringTitle(NodeSystem.Node node) {
        ...
        public void AutoConnect(NodeSystem.Node node) {
            if (autoConnectOutput == null) return;

            // Find input port of same type
            NodeSystem.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
