...
namespace Josephus.NodeSystem.Editor
{
    ...
    /// renaming a <see cref="NodeSystem.NodeGraph"/> asset, it appears that sometimes the v2 AssetDatabase will swap which asset
    /// is the main asset (present at top level) between the <see cref="NodeSystem.NodeGraph"/> and one of its <see cref="NodeSystem.Node"/>
    /// sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    /// finds a case where a <see cref="NodeSystem.Node"/> has been made the main asset it will swap it back to being a sub-asset
