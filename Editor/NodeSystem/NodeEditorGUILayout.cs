...
namespace Josephus.NodeSystem.Editor
{
    ...
    public static class NodeEditorGUILayout
    {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options)
        {
            ...
            NodeSystem.Node node = property.serializedObject.targetObject as NodeSystem.Node;
            NodeSystem.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, NodeSystem.NodePort port, bool includeChildren = true, params GUILayoutOption[] options)
        {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, NodeSystem.NodePort port, bool includeChildren = true, params GUILayoutOption[] options)
        {
            if (property == null) throw new NullReferenceException();
            ...
            else
            {
                ...
                if (port.direction == NodeSystem.NodePort.IO.Input)
                {
                    ...
                    NodeSystem.Node.ShowBackingValue showBacking = NodeSystem.Node.ShowBackingValue.Unconnected;
                    NodeSystem.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == NodeSystem.Node.ShowBackingValue.Never ||
                        (showBacking == NodeSystem.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    if (dynamicPortList)
                    {
                        ...
                        NodeSystem.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : NodeSystem.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking)
                    {
                        case NodeSystem.Node.ShowBackingValue.Unconnected:
                            ...
                            break;
                        case NodeSystem.Node.ShowBackingValue.Never:
                            // Display a label
                            EditorGUILayout.LabelField(label != null ? label : new GUIContent(property.displayName));
                            break;
                        case NodeSystem.Node.ShowBackingValue.Always:
                ...
                else if (port.direction == NodeSystem.NodePort.IO.Output)
                {
                    ...
                    NodeSystem.Node.ShowBackingValue showBacking = NodeSystem.Node.ShowBackingValue.Unconnected;
                    NodeSystem.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    {
                        Type type = GetType(property);
                        NodeSystem.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : NodeSystem.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking)
        ...
        public static void PortField(NodeSystem.NodePort port, params GUILayoutOption[] options)
        {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, NodeSystem.NodePort port, params GUILayoutOption[] options)
        {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, NodeSystem.NodePort port)
        {
            if (port == null) return;
        ...
        public static void AddPortField(NodeSystem.NodePort port)
        {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(NodeSystem.NodePort input, NodeSystem.NodePort output)
        {
        ...
        public static bool IsInstancePortListPort(NodeSystem.NodePort port)
        {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, NodeSystem.NodePort.IO io, NodeSystem.Node.ConnectionType connectionType = NodeSystem.Node.ConnectionType.Multiple, NodeSystem.Node.TypeConstraint typeConstraint = NodeSystem.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null)
        {
        ...
        public static bool IsDynamicPortListPort(NodeSystem.NodePort port)
        {
        ...
        {
            NodeSystem.Node node = serializedObject.targetObject as NodeSystem.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x =>
            {
                ...
                return new { index = -1, port = (NodeSystem.NodePort)null };
            }).Where(x => x.port != null);
            List<NodeSystem.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<NodeSystem.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, NodeSystem.NodePort.IO io, NodeSystem.Node.ConnectionType connectionType, NodeSystem.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation)
        {
            ...
            list.drawElementCallback =
                ...
                {
                    NodeSystem.NodePort port = node.GetPort(fieldName + " " + index);
            ...
            list.onReorderCallback =
                (ReorderableList rl) =>
                {
                    ...
                    if (rl.index > reorderableListIndex)
                    {
                        ...
                        {
                            NodeSystem.NodePort port = node.GetPort(fieldName + " " + i);
                            NodeSystem.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) =>
                {
                    ...
                    if (io == NodeSystem.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, NodeSystem.Node.TypeConstraint.None, newName);
                ...
                (ReorderableList rl) =>
                {
                    ...
                    else
                    {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++)
                        {
                            ...
                            {
                                NodeSystem.NodePort other = dynamicPorts[k].GetConnection(j);
            ...
            if (hasArrayData)
            {
                ...
                while (dynamicPortCount < arrayData.arraySize)
                {
                    ...
                    if (io == NodeSystem.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
