...
namespace Josephus.NodeSystem.Editor
{
    ...
    [CustomNodeGraphEditor(typeof(NodeSystem.NodeGraph))]
    public class NodeGraphEditor : NodeSystem.Editor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, NodeSystem.NodeGraph>
    {
        ...
        public virtual string GetNodeMenuName(Type type)
        {
            ...
            NodeSystem.Node.CreateNodeMenuAttribute attrib;
        ...
        public virtual void AddContextMenuItems(GenericMenu menu)
        {
            ...
            for (int i = 0; i < nodeTypes.Length; i++)
            {
                ...
                {
                    NodeSystem.Node node = CreateNode(type, pos);
        ...
        public virtual Gradient GetNoodleGradient(NodeSystem.NodePort output, NodeSystem.NodePort input)
        {
        ...
        /// <param name="output"> The output this noodle comes from. Never null. </param>
        /// <param name="input"> The output this noodle comes from. Can be null if we are dragging the noodle. </param>
        public virtual float GetNoodleThickness(NodeSystem.NodePort output, NodeSystem.NodePort input)
        {
            return 5f;
        }

        public virtual NoodlePath GetNoodlePath(NodeSystem.NodePort output, NodeSystem.NodePort input)
        {
            return NodeEditorPreferences.GetSettings().noodlePath;
        }

        public virtual NoodleStroke GetNoodleStroke(NodeSystem.NodePort output, NodeSystem.NodePort input)
        {
            return NodeEditorPreferences.GetSettings().noodleStroke;
        }

        /// <summary> Returned color is used to color ports </summary>
        public virtual Color GetPortColor(NodeSystem.NodePort port)
        {
        ...
        public virtual string GetPortTooltip(NodeSystem.NodePort port)
        {
        ...
        public virtual NodeSystem.Node CreateNode(Type type, Vector2 position)
        {
            Undo.RecordObject(target, "Create Node");
            NodeSystem.Node node = target.AddNode(type);
        ...
        public virtual NodeSystem.Node CopyNode(NodeSystem.Node original)
        {
            Undo.RecordObject(target, "Duplicate Node");
            NodeSystem.Node node = target.CopyNode(original);
        ...
        public virtual bool CanRemove(NodeSystem.Node node)
        {
            // Check graph attributes to see if this node is required
            Type graphType = target.GetType();
            NodeSystem.NodeGraph.RequireNodeAttribute[] attribs = Array.ConvertAll(
                graphType.GetCustomAttributes(typeof(NodeSystem.NodeGraph.RequireNodeAttribute), true), x => x as NodeSystem.NodeGraph.RequireNodeAttribute);
        ...
        public virtual void RemoveNode(NodeSystem.Node node)
        {
        ...
        NodeSystem.Editor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, NodeSystem.NodeGraph>.INodeEditorAttrib
        {
            ...
            public CustomNodeGraphEditorAttribute(Type inspectedType, string editorPrefsKey = "NodeSystem.Settings")
            {
